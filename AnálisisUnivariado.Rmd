...
# Renombrar columnas de base_saludmental con nombres cortos y consistentes
...
```{r , echo=TRUE}
# Crear un vector con los nombres de las columnas que quieres analizar
variables <- c(
  "confianza_vecinos", "hablar_vecinos_antes", "freq_hablar_vecinos_antes",
  "hablar_amigos_familia", "hablar_amigos_familia_antes",
  "cumple_autocuidado", "familia_autocuidado", "vecinos_autocuidado",
  "info_alcaldia_clara", "info_alcaldia_suficiente", "info_alcaldia_veridica",
  "info_alcaldia_oportuna", "info_alcaldia_contradictoria",
  "medios_info_alcaldia", "sabe_actuar_covid", "info_suficiente_covid",
  "info_riesgos_covid", "diagnosticado_covid"
...
# Revisar si la variable existe en el dataset
...
# Revisar si la tabla tiene más de 0 elementos
...
# Convertir a data frame solo si hay más de 1 categoría

tabla_df <- as.data.frame(tabla)

if (ncol(tabla_df) == 1) {
names(tabla_df) <- "Frecuencia"
} else {
...
# Graficar si hay 10 o menos categorías

if (nrow(tabla_df) <= 10) {
barplot(tabla_df$Frecuencia, names.arg = ifelse(ncol(tabla_df) == 2, tabla_df$Categoría, 1:nrow(tabla_df)),
main = var, col = "steelblue", las = 2)
} else {
